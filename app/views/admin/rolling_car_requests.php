...
<section class="requests_section">
	...
	<form class="requests_form">
		<span>Заявки на авто в прокат</span>
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						...
						<td data-label="Статус"><div><a onclick="sendAjax('/admin/rolling_car_requests', $(this),<?= $requests[$key]['id'] ?>)"data-id="<?= $requests[$key]['id'] ?>" data-status="<?= $requests[$key]['status'] ?>" class="check_request buy_request"><i class="fas fa-check"></i></a><?= \Core\Url::getLink("/admin/rolling_car_request_delete", '<i class="far fa-trash-alt"></i>', array('id'=>$requests[$key]['id'])); ?></div></td>
					</tr>
					<?php } ?>	
				</tbody>
			</table>
